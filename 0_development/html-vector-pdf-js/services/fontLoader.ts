...
const FONT_CDN_URLS: Record<string, FontDefinition> = {
    ...
    /**** AMENDMENT [start] "Add Carlito font definition" ****/
    Carlito: {
        name: 'Carlito',
        url: 'EMBEDDED',
        format: 'truetype'
...
/**** AMENDMENT [start] "Add family-based font detector" ****/
/**
 * Detect required fonts from CSS font-families
 * @param families - Array of CSS font-family strings
 * @returns Set of required font names
 */
export const detectRequiredFontsFromFamilies = (families: string[]): Set<string> => {
    const requiredFonts = new Set<string>();

    for (const fam of families) {
        if (!fam) continue;
        const lowFam = fam.toLowerCase();
        if (lowFam.includes('notosanssc')) requiredFonts.add('NotoSansSC');
        if (lowFam.includes('notosansjp')) requiredFonts.add('NotoSansJP');
        if (lowFam.includes('notosanskr')) requiredFonts.add('NotoSansKR');
...
/**** AMENDMENT [end] "Add family-based font detector" ****/
